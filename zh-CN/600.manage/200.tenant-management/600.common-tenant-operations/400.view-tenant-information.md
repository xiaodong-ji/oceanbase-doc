...
## 查看租户各资源组的参数及实时 IO 统计信息

sys 租户通过 `GV$OB_GROUP_IO_STAT` 视图，可以实时查看所有 OBServer 节点上每个资源组的参数及实时的 IOPS 和带宽统计信息，示例如下：
...
+-----------+----------------+----------+----------+-------------------+-------+---------------------+---------------------+-----------+---------------------+---------------------------+--------------------+----------------------------+
|      1002 | 172.xx.xxx.xxx |     2882 |        0 | OTHER_GROUP       | READ  | 9223372036854775807 | 9223372036854775807 |         0 | 9223372036854775807 | unlimited                 |                  0 | 0B/s                       |
|      1002 | 172.xx.xxx.xxx |     2882 |        0 | OTHER_GROUP       | WRITE | 9223372036854775807 | 9223372036854775807 |         0 | 9223372036854775807 | unlimited                 |                  0 | 0B/s                       |
|      1002 | 172.xx.xxx.xxx |     2882 |    10000 | interactive_group | READ  | 9223372036854775807 | 9223372036854775807 |         0 | 9223372036854775807 | unlimited                 |                  0 | 0B/s                       |
|      1002 | 172.xx.xxx.xxx |     2882 |    10000 | interactive_group | WRITE | 9223372036854775807 | 9223372036854775807 |         0 | 9223372036854775807 | unlimited                 |                  0 | 0B/s                       |
|      1002 | 172.xx.xxx.xxx |     2882 |    10001 | batch_group       | READ  | 9223372036854775807 | 9223372036854775807 |         0 | 9223372036854775807 | unlimited                 |                  0 | 0B/s                       |
+-----------+----------------+----------+----------+-------------------+-------+---------------------+---------------------+-----------+---------------------+---------------------------+--------------------+----------------------------+
5 rows in set
```

从第三行结果可知，租户 `1002` 在 `172.xx.xxx.xxx` 节点上，资源组 `interactive_group` 中配置的读操作的最小 IOPS 为， 最大 IOPS 为，当前实际的 IOPS 为，配置的最大网络带宽为 ，当前网络带宽为。 
...
+----------------+----------+-----------+------------------+--------------+------+-----------+-----------+-------------+-------------+----------+
| 172.xx.xxx.xxx |     2882 |      1002 | DEFAULT_FUNCTION | LOCAL READ   |    0 |         0 |         0 |           0 |           0 |        0 |
| 172.xx.xxx.xxx |     2882 |      1002 | DEFAULT_FUNCTION | LOCAL WRITE  | 4096 |         2 |         0 |          20 |         142 |      178 |
| 172.xx.xxx.xxx |     2882 |      1002 | DEFAULT_FUNCTION | REMOTE READ  |    0 |         0 |         0 |          20 |         142 |      178 |
| 172.xx.xxx.xxx |     2882 |      1002 | DEFAULT_FUNCTION | REMOTE WRITE |    0 |         0 |         0 |          20 |         142 |      178 |
| 172.xx.xxx.xxx |     2882 |      1002 | COMPACTION_HIGH  | LOCAL READ   |    0 |         0 |         0 |          20 |         142 |      178 |
+----------------+----------+-----------+------------------+--------------+------+-----------+-----------+-------------+-------------+----------+
5 rows in set
```

从第五行结果可知，租户 `1002` 在 `172.xx.xxx.xxx` 节点上，Mini Merge 和 DDL KV Merge 等任务（`COMPACTION_HIGH`）本地读的平均 IO（`SIZE`）大小为 0，实际 IOPS（`REAL_IOPS`） 为 0， 实际所使用的带宽（`REAL_MBPS`）为 0 MB/S，IO 调度的平均消耗时间（`SCHEDULE_US`）为 20 us，磁盘 IO 的平均执行时间（`IO_DELAY_US`）为 142 us，IO 的总体平均执行时间（`TOTAL_US`）为 178 us。
