...
从第五行结果可知，租户 `1002` 在节点 `172.xx.xxx.xxx` 上，Mini Merge 和 DDL KV Merge 等任务（`COMPACTION_HIGH`）本地读的平均 IO（`SIZE`）大小为 0，实际 IOPS（`REAL_IOPS`） 为 0， 实际所使用的带宽（`REAL_MBPS`）为 0 MB/S，IO 调度的平均消耗时间（`SCHEDULE_US`）为 20 us，磁盘 IO 的平均执行时间（`IO_DELAY_US`）为 142 us，IO 的总体平均执行时间（`TOTAL_US`）为 178 us。
