...
2. 配置归档目的端。
   ...
   * 配置 `LOCATION` (必选)
       ...
       `LOCATION` 属性用于指定归档目的端，目前 OceanBase 数据库支持的归档目的端介质为 NFS、阿里云 OSS、腾讯云 COS、AWS S3 以及兼容 S3 协议的对象存储，例如：华为 OBS、Google GCS。部分备份介质可能需要满足一些基本要求才能使用，有关各备份介质的具体要求，参见 [物理备份与恢复概述](../100.overview-of-physical-backup-and-recovery.md) 中的 **备份介质要求**。
      ...
      * `tagging`：表示清理模式为对满足清理要求的归档文件设置 `Tag`，归档文件将仍然保留。
      ...
      <main id="notice" type='notice'>
      <h4>注意</h4>
      <p>COS 对象存储作为日志归档目的端时，通过 COS 协议访问 COS 对象存储的方式即将废弃，建议使用兼容 S3 协议的访问方式来访问 COS 对象存储。</p>
      </main>
      ...
      * 使用 COS 作为归档介质时，用户租户为本租户设置归档目的端并配置为 `delete` 清理模式。
      ...
      与 OSS 一样，AWS S3 也支持通过 `delete_mode` 参数来配置归档文件的清理模式，配置方法与 OSS 一样。
      ...
      * 使用 GCS 作为归档介质时，用户租户为本租户设置归档目的端。
      ...
      3. 设置归档目的端并通过 `delete_mode` 参数来配置归档文件的清理模式。

          使用 COS 作为归档介质时，系统租户为指定租户 `mysql_tenant` 设置归档目的端并配置为 `delete` 清理模式的示例如下：
          ...
          使用 COS 作为归档介质时，用户租户为本租户设置归档目的端并配置为 `delete` 清理模式的示例如下：
          ...
          示例中，`s3://` 使用 S3 协议访问归档目的端，存储桶名 `oceanbase-test`，在存储桶中的路径是 `/backup/archive`，同时使用 `?` 来分隔路径的其他参数，`host` 用于设置 COS 的主机地址，即 endpoint（不带存储桶名的地址），`access_id` 和 `access_key` 用于设置 `COS` 的访问密钥，清理模式设置为 `delete`。
