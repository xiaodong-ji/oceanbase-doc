...
OceanBase 数据库表自动分区分裂支持表级别和租户级别的自动分区分裂配置。
...
* 不支持自动分裂 Interval、List、Hash 分区表。
* 不支持自动分裂二级分区表。
...
```sql
CREATE TABLE table_name (column_name column_type[, column_name column_type ...], PRIMARY KEY(column_name[, column_name ...]))
    PARTITION BY RANGE [COLUMNS]([column_name[, column_name ...]]) [SIZE(['size_value'])]
...
| PRIMARY KEY          | 指定表的主键。|
| PARTITION BY RANGE() | 指定创建自动分区的表。可以指定预分区键，当触发自动分区时，会将填写的预分区键作为实际的分区键（若不填写，则使用主键为分区键）。<main id="notice" type='notice'><h4>注意</h4><p>如果是指定多列分区键，需用使用 <code>PARTITION BY RANGE COLUMNS()</code>。</p></main>|
| SIZE(['size_value']) | 可选项，指定分区的分裂阈值。`SIZE()` 子句或 `size_value` 缺省时，默认读租户级别配置项。`size_value` 可取值 `unlimited`，表示分裂阈值无限制，不会分裂。|
| PARTITION partition_name VALUES LESS THAN (value[, value ...]) | 可选项，预先创建分区位点。|
...
|       **参数**       | **说明** |
|----------------------|----------|
|column_name[, column_name ...] | 修改非分区表的预分区键。在使用 `ALTER TABLE` 语句修改非分区表的自动分区属性时，若 `RANGE()` 内填写了预分区键信息，则会修改 Schema 中的相应记录；如果没有填写信息，则代表保持原有设定。<main id="notice" type='notice'><h4>注意</h4><p>如果修改为多列分区键，需用使用 <code>PARTITION BY RANGE COLUMNS()</code>。</p></main>|
| size_value | 可选项，修改表的自动分区阈值。当修改未 `unlimited` 时，不影响已有分区，后续不再调度新的自动分区分裂。|
| PARTITION partition_name VALUES LESS THAN (value[, value ...]) | 可选项，修改分区规则。若未使用 SIZE 语句但又给定了分区定义，则仅修改分区规则，不影响原有的自动分区定义。|
...
* 添加分区：跟普通表一样，如果是非分区表，那么无法添加分区，如果是分区表，那么能添加符合分区规则的分区。
* 删除分区：删除自动分裂和手动添加的分区都支持。

    <main id="notice" type='explain'>
      <h4>说明</h4>
      <p>当删除分区/Truncate 分区时，全局索引会进行一次预分裂。</p>
    </main>

* 分区分裂：分裂自动分裂和手动添加的分区都支持。
* 分区归并（暂不支持）：归并自动分裂和手动添加的分区。
* 分区交换：只要满足分区的条件，就能交换。
* 修改分区规则：大部分不支持，比如一级转换成二级，修改成 Hash 分区等等，只有修改成一级的按照主键Range 分区的才行。
* 修改主键：修改完主键时，按照新主键进行一次自动分区的拆分，如果用户配置了自动分区键，需要满足修改主键后自动分区键是主键前缀。
* 创建全局索引：会进行一次预分裂。
* 修改列类型、修改字符集、修改分区规则：如果表上有全局索引，会对全局索引进行一次预分裂。

### 预分区键处理

* 自动分区的非分区表会将预分区键记录在 Schema 中。若用户没有填写预分区键信息，在触发自动分区时，主表将会取当前主键作为自己的分区键，全局索引则会取自己的索引键为分区键。
* 涉及到列变更的相关 DDL，如修改列名、修改列属性、修改主键等操作，都需要将预分区键视为分区键进行限制或特殊处理。
* 创建局部唯一索引时，存在索引键需要覆盖分区键对约束。需要将预分区键视为分区键进行检测。
...
2. 创建自动分区的非分区表 `test1_tbl1`，分裂阈值为 10GB。

    ```sql
    CREATE TABLE test1_tbl1 (col1 INT, col2 INT, col3 VARCHAR(30), col4 VARCHAR(50),
        PRIMARY KEY(col1, col2))
        PARTITION BY RANGE() SIZE('10GB');
    ```

3. 在创建自动分区的非分区表 `test1_tbl2` 时，指定预分区键 `col1`。

    ```sql
    CREATE TABLE test1_tbl2 (col1 INT, col2 INT, col3 VARCHAR(30), col4 VARCHAR(50),
        PRIMARY KEY(col1, col2))
        PARTITION BY RANGE(col1) SIZE('1GB');
    ```

4. 在创建自动分区的非分区表 `test1_tbl3` 时，指定预分区键 `col1, col2`，并预先创建分区位点。

    ```sql
    CREATE TABLE test1_tbl3 (col1 INT, col2 INT, col3 VARCHAR(30), col4 VARCHAR(50),
        PRIMARY KEY(col1, col2))
...
5. 查看表的自动分区属性。
    ...
    | db_test      | test1_tbl2 | TRUE       |             1073741824 |
    | db_test      | test1_tbl3 | TRUE       |            10737418240 |
    +--------------+------------+------------+------------------------+
    3 rows in set
    ```
...
1. 创建自动分区的分区表 `test2_tbl1`。

    ```sql
    CREATE TABLE test2_tbl1 (col1 INT, col2 INT, PRIMARY KEY(col1))
...
3. 修改分区表 `test2_tbl1` 的自动分区属性及分区定义。

    ```sql
    ALTER TABLE test2_tbl1
        PARTITION BY RANGE (col1) SIZE('5GB')
            (PARTITION p0 VALUES LESS THAN(100),
             PARTITION p_max VALUES LESS THAN (MAXVALUE));
    ```

4. 修改分区表 `test2_tbl1` 的分区定义，不修改自动分区属性。未使用 SIZE 语句但又给定了分区定义，则仅修改分区规则，不影响原有的自动分区定义。
...
5. 禁止表 `test2_tbl1` 进行自动分区，修改后不影响已有分区，后续不再调度新的自动分区分裂。

    ```sql
    ALTER TABLE test2_tbl1 PARTITION BY RANGE() SIZE('unlimited');
    ```

6. 创建自动分区的非分区表 `test2_tbl2`。

    ```sql
    CREATE TABLE test2_tbl2 (col1 INT, col2 INT, PRIMARY KEY(col1, col2))
        PARTITION BY RANGE(col1) SIZE('10GB');
    ```

7. 将表 `test2_tbl2` 的预分区键由 `col1` 改成 `col1,col2`。

    ```sql
    ALTER TABLE test2_tbl2 PARTITION BY RANGE COLUMNS(col1,col2) SIZE('10GB');
    ```

8. 查看表 `test2_tbl2` 的结构和定义。

    ```sql
    SHOW CREATE TABLE test2_tbl2;
    ```
    ...
    partition by range columns(col1,col2) size ('10240MB') |
    +------------+------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
    1 row in set
    ```
...
9. 查看表的自动分区属性。
    ...
    | TABLE_SCHEMA | TABLE_NAME | AUTO_SPLIT | AUTO_SPLIT_TABLET_SIZE |
    +--------------+------------+------------+------------------------+
    | db_test      | test2_tbl1 | FALSE      |                      0 |
    | db_test      | test2_tbl2 | TRUE       |            10737418240 |
    +--------------+------------+------------+------------------------+
    2 rows in set
    ```
...
3. 创建自动分区的非分区表 `test3_tbl2`（使用默认配置的 128MB 为分裂阈值）。
