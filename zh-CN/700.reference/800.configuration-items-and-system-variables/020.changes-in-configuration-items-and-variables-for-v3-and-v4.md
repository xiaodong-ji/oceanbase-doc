...
| 配置项名                                                                                                       | 引入版本    | 描述                             |
|---------------------------------------------------------------------------------------------------------------|-------------|---------------------------------|
| ob_encoding_granularity                                                            | V4.3.3                  | 用于控制向量索引模块能使用内存资源的上限百分比。 |
| major_compaction_memory_percentage_limit                                           | V4.3.3                  | 用于指合并使用的内存占租户内存的比例上限。|
| ob_storage_s3_url_encode_type                                                      | V4.3.3                  | 设置 S3 发送请求时的 URL 编码方式是否兼容 Rfc3986 标准。|
| ob_vector_memory_limit_percentage                                                  | V4.3.3                  | 配置向量索引模块对内存资源占用的阈值（单位为百分比）。   |
...
| 变量名                          | 描述                                      |
|---------------------------------|-------------------------------------------|
| [ob_default_lob_inrow_threshold](200.system-variable/400.session-system-variable/6300.ob_default_lob_inrow_threshold-session.md)      | <ul><li>对于 V4.3.x 版本，从 V4.3.2 版本开始，默认值由 4096 调整为 8192。</li> <li>对于 V4.2.x 版本，从 V4.2.3 版本开始，默认值由 4096 调整为 8192。</li></ul> |
...
| 变量名                                                                               | 引入版本    | 描述                             |
|-------------------------------------------------------------------------------------|-------------|---------------------------------|
| ob_hnsw_ef_search  | V4.3.3 | 于设定 HNSW 向量索引搜索时考虑的邻居节点数。 |
| ob_table_access_policy| V4.3.3| 控制优化器生成基表计划时优先选择特定的存储格式。|
